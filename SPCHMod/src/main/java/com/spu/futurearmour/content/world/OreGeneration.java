package com.spu.futurearmour.content.world;
...
public class OreGeneration {
    public static void generateOres(final BiomeLoadingEvent event) {
        if (!(event.getCategory().equals(Biome.Category.THEEND) || event.getCategory().equals(Biome.Category.NETHER))) {
            ...
            generateOre(event.getGeneration(),
                    OreFeatureConfig.FillerBlockType.NATURAL_STONE,
                    BlockRegistry.TITANIUM_ORE.get().defaultBlockState(),
                    3,3, 19, 3);

            generateOre(event.getGeneration(),
                    OreFeatureConfig.FillerBlockType.NATURAL_STONE,
                    BlockRegistry.URANIUM_ORE.get().defaultBlockState(),
                    2,5, 20, 7);

            generateOre(event.getGeneration(),
                    OreFeatureConfig.FillerBlockType.NATURAL_STONE,
                    BlockRegistry.MAGNESIUM_ORE.get().defaultBlockState(),
                    7,9, 26, 4);
        }
    }
